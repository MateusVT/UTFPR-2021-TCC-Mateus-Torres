...
def getRepData(filePath):
    ...
    with open(filePath, 'r', encoding='utf8') as file:
        ...
        creationDate = datetime.strptime(reader['created_at'].split('T')[0], '%Y-%m-%d').date()
        ...
        if reader['has_newcomer_labels']:
            try:
                if len(reader['weekly_distribuition_before']) >= 1 and len(reader['weekly_distribuition_after']) >= 1:#Condicional para identificar projetos com dados insuficientes para comparação
                    labels=True
                #else:
                    #print(owner,repName)
            except(KeyError):
                print("error -:> ", owner,repName)
                pass

    return (repFullName, owner, repName, language, creationDate, weekly_dist, weekly_dist_len, first_contributions_len, stars_size, subs_size, labels)
...
def getPrePostData(filePath):
    with open(filePath, 'r', encoding='utf8') as file:
        ...
        try:
            ...
            print('ERROR: No pre/post data available: ',owner,repName) # Caso dê algum problema na leitura dos dados
            pass
...
for file in files:
    repFullName, owner, repName, language, creationDate, weekly_dist, nWD, nFC, stars_size, subs_size, labels = getRepData(file)
    dataAll[repFullName] = {'owner':owner, 'lang':language, 'creationDate':creationDate, 'weekly_dist':weekly_dist, 'nWD':nWD, 'nFC':nFC, 'stars':stars_size, 'subs':subs_size}

    if labels:
        label, dataTreatment, dataControl, invalid = getPrePostData(file)
        dataLabel[repFullName] = {'owner':owner, 'lang':language, 'creationDate':creationDate, 'weekly_dist':weekly_dist, 'nWD':nWD, 'nFC':nFC, 'stars':stars_size, 'subs':subs_size, 'label': label, 'treatment':dataTreatment, 'control':dataControl, 'invalid':invalid}
    else:
        dataNoLabel[repFullName] = {'owner':owner, 'lang':language, 'creationDate':creationDate, 'weekly_dist':weekly_dist, 'nWD':nWD, 'nFC':nFC, 'stars':stars_size, 'subs':subs_size}
...
def generateDataByMedian(dataDict, params=['creationDate', 'nWD', 'stars', 'subs'], outName='resulByMedians.csv'):
    with open(outName, 'w') as outFile:
        for param in params:
